...
1. **Elenme analizi** — "Hangi aşamada eleniyorum?" sorusunu görselleştiren içgörü ekranı. Türkiye'de bu içgörüyü sunan başka uygulama yok.
2. **Türkçe ve yerel platform organizasyonu** — Kariyer.net, Youthall, Anbean ve LinkedIn başvurularını tek arşivde tutar. Türkçe arayüz, Türk iş arama kültürüne göre tasarlanmıştır.
...
- Otomatik bilgi çekme (link yapıştırma ile şirket/pozisyon otomatik doldurma) — Faz 0.2 teknik uygunluk araştırmasında Kariyer.net'in sunucu taraflı bot koruması (PerimeterX) Supabase Edge Function IP'lerini engellediği için kapsam dışı bırakıldı. Yeniden değerlendirme: v2'de proxy servisi veya resmi API entegrasyonu araştırılacak.
...
> Otomatik bilgi çekme özelliği Faz 0.2 araştırması sonucunda MVP kapsamından çıkarıldı (bkz. §12 Kapsam Dışı Kararlar). Sprint 1 manuel ekleme akışına odaklanır.
...
|------|---------|------|-------------------|
| ~~#1 Otomatik bilgi çekme teknik riski~~ | Gerçekleşti | — | Faz 0.2 araştırması sonrası v2'ye ertelendi (bkz. §12) |
...
> **Güncelleme (Mayıs 2026):** Faz 0.2 araştırması tamamlandı. Kariyer.net ilan sayfaları server-side render edilmesine rağmen, sunucu PerimeterX bot koruması nedeniyle Supabase Edge Function IP'lerini HTTP 403 ile engelliyor. **Kırmızı senaryo gerçekleşti.** Otomatik bilgi çekme özelliği MVP kapsamından çıkarıldı, manuel ekleme akışı öne çekildi. Detay için §12.
...
| Kariyer.net / Youthall / Anbean otomatik bilgi çekme yok | Kariyer.net'in PerimeterX bot koruması veri merkezi IP'lerini engelliyor (Faz 0.2 araştırma sonucu — HTTP 403) | v2: proxy servisi (ScrapingBee, Bright Data) veya resmi API ortaklığı araştırılacak |
